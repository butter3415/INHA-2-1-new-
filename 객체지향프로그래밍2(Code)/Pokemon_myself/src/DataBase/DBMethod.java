...
public class DBMethod extends DBManager{
    ...
    public static void winPokemonInsertDictionary(int ranNum){
        try{
            stm.executeUpdate("UPDATE pokemondbstudy.wildpokemondictionary " +
                    "SET " +
                    "`pokemon_dictionary_id` = " + ranNum + ", " +
                    "`name` = (SELECT `name` FROM pokemondbstudy.pokemondatabase where `id` = " + ranNum +  ")," +
                    "`skill1` = (SELECT `skill1` FROM pokemondbstudy.pokemondatabase where `id` = " + ranNum + ")," +
                    "`skill2` = (SELECT `skill2` FROM pokemondbstudy.pokemondatabase where `id` = " + ranNum + ")" +
                    "WHERE id = " + ranNum);
...
//    public static void selectWildPokemonPowerNSpeed(int wildRan){
//        try{
//            ResultSet rs = stm.executeQuery("SELECT power, speed FROM `pokemonDataBase` WHERE id = " + wildRan);
//
//        } catch(Exception e){
//            e.printStackTrace();
//        }
